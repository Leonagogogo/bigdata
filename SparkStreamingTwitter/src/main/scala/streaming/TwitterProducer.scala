...
object TwitterProducer {
  def main(args: Array[String]) {
    ...
    def getSentiment(sentiment: Int): Integer = sentiment match {
      case x if x == 0 || x == 1 => 0
      case 2 => 3
      case x if x == 3 || x == 4 => 5
    }
    ...
    tweets.filter(x => x.getLang == "en").map(_.getText).map(tweetText => (tweetText, wordsOf(tweetText)))
    ...
    textAndSentences.foreachRDD( rdd => {
      rdd.foreachPartition( partition => {
        ...
        val pipleprops: Properties = new Properties()
        pipleprops.put("annotators", "tokenize, ssplit, parse, sentiment")
        val pipeline: StanfordCoreNLP = new StanfordCoreNLP(pipleprops)

        partition.foreach( record => {
          val data = record.toString
          val document1: Annotation = new Annotation(data)
          ...
          val result = sentences1
            .map(sentence => (sentence, sentence.get(classOf[SentimentAnnotatedTree])))
            .map {case (sentence, tree) => (getSentiment(RNNCoreAnnotations.getPredictedClass(tree))) }
            .toString()

          //   val positive = result.map(x => (x==0))
          //  val negative = result.map((x => (x==5)))

          val message = new ProducerRecord[String,String](topic, null, result)
